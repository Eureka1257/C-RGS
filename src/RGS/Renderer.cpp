...
namespace RGS
{
	bool Renderer::IsVertexVisible(const Vector4& clipPos) 
	{
	...
	bool Renderer::IsInsidePlane(const Vector4& clipPos, const Plane plane)
	{
		switch (plane)
		{
			//w必须是个正数
	...
	float Renderer::GetIntersectRatio(const Vector4& prev, const Vector4 curr, const Plane plane)
	{
		switch (plane)
		{
	...
	{
		return weights[0] >= -EPISILON && weights[1] >= -EPISILON && weights[2] >= -EPISILON;
	}

	Renderer::BoundingBox Renderer::GetBoundingBox(const Vector4(&fragCoords)[3],
		const int width,
		const int height)
	{
	...
	void Renderer::CaculateWeights(float(&screenWeights)[3],
		float(&weights)[3],
		const Vector4(&fragCoords)[3],
		const Vector2& screenPoint)
	{
		Vector2 ab = fragCoords[1] - fragCoords[0];
		Vector2 ac = fragCoords[2] - fragCoords[0];
		Vector2 ap = screenPoint - fragCoords[0];
